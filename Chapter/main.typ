...
*Fig. 1.* Overview of the pangenome workflow described in this protocol. The pipeline proceeds in three stages. *First (top)*: genome assemblies were generated from 10x Genomics Linked-Read sequencing of 31 HXB/BXH recombinant inbred strains derived from the parental SHR/OlaIpcv (H) and BN-Lx/Cub (B) strains; six representative strains (HXB23, BXH6, HXB21, SHROlaIpcv, BXH3, and BNLxCub) are shown. Assembly completeness is assessed with Compleasm (%BUSCOs).
*Second (middle)*: chromosome-level pangenome graphs were built using PGGB; an example of the chr12 pangenome graph is shown, visualized with odgi viz, whefre each horizontal colored line represents a haplotype and vertical white stripes indicate structural variation.
*Third (bottom*): downstream analyses including (i) variant calling from graph snarls (illustrated with a two-genome bubble-graph schematic showing divergent paths through shared anchor nodes);(ii) read mapping with vg Giraffe (shown with short reads aligned to a similar bubble-graph);(iii) variant benchmarking comparing the pangenome dataset against a genomic truth-set to capture pangenome-only, genomic-only, and shared variants, evaluated by precision, recall, and F1-score; (iv) phenome-wide association study (PheWAS) linking validated SNPs to phenotypes including blood glucose concentration and central nervous system traits via GeneNetwork; (v) structural variant (SV) validation using Nanopore adaptive sequencing to confirm a representative 82 bp insertion at chr12:1,942,010 across samples relative to the reference genome; (vi) SV interpretation showing haplotype-resolved allele frequencies across strains. A representative complex insertion of 82 bp in the Cd209c gene, supported by all four calling methods, resolves into six haplotypes composed of seven variable blocks, of which four are observed fewer than two times.